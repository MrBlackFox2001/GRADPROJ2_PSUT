...
def predict_xml(xml_file):
    ...
    predicted_label_index = np.argmax(predictions)
    class_labels = ["N", "VEB", "The image shows a Ventricular heartbeat.` These heartbeats often occur when the normal electrical conduction system of the heart is disrupted or when the SA and AV nodes fail to initiate and conduct impulses properly.", "Fusion"]
    predicted_label = class_labels[predicted_label_index]
    print(f"The predicted label is: {predicted_label}")
    return predicted_label
...
def predict_raw(csv_file_path):
    ...
    class_labels = ["N", "The image shows a Ventricular heartbeat", "SVEB", "Fusion"]
